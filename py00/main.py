...
def test_ft_exercise(exercise_file_name, exercise_dir=None):
    """
    This function tries to run one of your exercises.

    For example: test_ft_exercise("ft_plot_area", "ex2") will:
    - Look for a file called ft_plot_area.py in ex2/ directory
    - Import it
    ...
    try:
        # Import your exercise file from the appropriate directory
        import sys
        import importlib.util
        import os
        
        if exercise_dir is None:
            exercise_dir = f"ex{exercise_file_name[-1]}"  # Default fallback
        
        # Get the directory where main.py is located
        main_dir = os.path.dirname(os.path.abspath(__file__))
        file_path = os.path.join(main_dir, exercise_dir, f"{exercise_file_name}.py")
